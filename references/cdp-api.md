...
- 启动：`node ~/.claude/skills/super-browser/scripts/cdp-proxy.mjs &`
...
用于捕获 XHR/Fetch 请求，直接获取 API 响应数据。

### POST /network/start?target=ID&pattern=

开始监控指定 tab 的网络请求。`pattern` 支持 `*` 通配符，为空时捕获所有请求。

```bash
# 监控所有请求
curl -s -X POST "http://localhost:3456/network/start?target=ID"

# 只监控 /api/ 开头的请求
curl -s -X POST "http://localhost:3456/network/start?target=ID&pattern=*/api/*"
```

### GET /network/requests?target=ID&pattern=

获取已捕获的请求列表。可再次指定 `pattern` 过滤。

```bash
curl -s "http://localhost:3456/network/requests?target=ID"
```

### POST /network/stop?target=ID

停止监控并清除缓存。

```bash
curl -s -X POST "http://localhost:3456/network/stop?target=ID"
```
